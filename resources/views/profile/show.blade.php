<x-user-layout>
    <div class="flex items-start ml-10 mt-10">
        ...
        <div class="ml-4">
            <span class="text-black font-bold text-2xl">{{ $user->name }}</span>
            <span class="text-gray-700 block">{{ $user->email }}</span>
            <span class="text-gray-500 text-sm block">
                Membre depuis le {{ $user->created_at->isoFormat('LL') }}
            </span>
            <!-- User Bio -->
            <div class="text-gray-700 mt-5 w-1/3 mb-5">
                {{ $user->bio }}
            </div>
    ...
    @if(auth()->user()->id === $user->id)
        <a href="{{ route('profile.update') }}" class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 mt-4">
            Modifier le profil
        </a>
    @elseif(auth()->user()->id !== $user->id)
        @if(auth()->user()->following->contains($user))
            <form action="{{ route('profile.unfollow', $user) }}" method="post">
                @csrf
                <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md mt-4">
                    Ne plus suivre
                </button>
            </form>
        @else
            <form action="{{ route('profile.follow', $user) }}" method="post">
                @csrf
                <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-700 mt-4">
                    Suivre
                </button>
            </form>
        @endif
